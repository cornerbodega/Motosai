...
eventManager.add(element, 'click', this.handleClick, {
  bindContext: this
}, 'myComponent');
...
const geometry = pool.getSharedGeometry('box', { width: 1, height: 1, depth: 1 });
...
**Usage:**
```javascript
import { disposeObject3D, disposeScene } from './refactoring/src/utils/DisposalUtils.js';
...
```javascript
import { memoryLeakDetector, patchThreeJSForTracking } from './refactoring/src/utils/DisposalUtils.js';
...
console.log('Geometry leaks:', stats.geometryLeaks);
console.log('Material leaks:', stats.materialLeaks);
console.log('Texture leaks:', stats.textureLeaks);
```
...
console.log('Event listeners:', eventStats.totalListeners);

// AudioManager stats
const audioStats = getGlobalAudioManager().getStats();
console.log('Active sounds:', audioStats.activeSounds);
```
