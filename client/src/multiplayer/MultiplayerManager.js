...
export class MultiplayerManager {
  ...
  async connect(username = null) {
    try {
      ...
      const usernameToUse =
        username ||
        storedUsername ||
        `Rider_${Math.random().toString(36).substring(7)}`;
  ...
  createOtherPlayerMotorcycle(username) {
    ...
    const riderColor = new THREE.Color().setHSL(
      (bikeColor.getHSL({}).h + 0.3) % 1,
      0.6,
      0.4
    ); // Complementary color
  ...
  addOtherPlayer(playerId, username) {
    ...
    const mainPlayerPos = this.game.physics?.getState()?.position || {
      x: 0,
      y: 0,
      z: 0,
    };
  ...
  handlePlayerCrash(data) {
    ...
    if (this.game && this.game.deathAnimation) {
      const crashPosition = new THREE.Vector3(
        data.position.x,
        data.position.y,
        data.position.z
      );
      const crashVelocity = new THREE.Vector3(
        data.velocity.x || 0,
        data.velocity.y || 0,
        data.velocity.z || 0
      );
      ...
      this.game.deathAnimation.triggerRemoteCrash(
        crashPosition,
        crashVelocity,
        data.username
      );
  ...
  handlePlayerDeath(data) {
    ...
    if (this.game && this.game.deathAnimation) {
      ...
      this.game.deathAnimation.triggerRemoteDeath(
        deathPosition,
        deathVelocity,
        data.username
      );
        ...
        if (player.isDead) {
          ...
          if (motorcycle.userData.body && motorcycle.userData.originalColor) {
            ...
            if (motorcycle.userData.flashTimer >= 200) {
              motorcycle.userData.body.material.color.copy(
                motorcycle.userData.originalColor
              );
              motorcycle.userData.flashTimer = 0;
            }
          }
          ...
          if (motorcycle.userData.rider) {
            ...
            motorcycle.userData.rider.rotation.x =
              (player.rotation.x || 0) * 0.3;
          }
  ...
  getPlayerList() {
    ...
    this.otherPlayers.forEach((data, id) => {
      ...
      if (id !== this.playerId) {
        ...
        console.warn(
          `⚠️ Found self (${id}) in otherPlayers map - this should not happen!`
        );
      }
    });
