...
export class MaterialTracker {
      ...
      if (THREE[materialType]) {
        ...
        try {
          ...
          if (descriptor && !descriptor.configurable) {
            console.warn(
              `Could not patch ${materialType}: property is not configurable (Safari)`
            );
            return;
          }
          ...
          const ProxyConstructor = new Proxy(OriginalConstructor, {
            construct(target, args) {
              ...
              if (window.materialTracker) {
                window.materialTracker.trackMaterialCreation(
                  instance,
                  materialType
                );
              }
  ...
  getStats() {
    const undisposed = Array.from(this.createdMaterials.values()).filter(
      (info) => !info.disposed
    );
  ...
  getUndisposedMaterials() {
    return Array.from(this.createdMaterials.values()).filter(
      (info) => !info.disposed
    );
  }
  ...
  async findLeakSource() {
    ...
    const recentLeaks = undisposed.filter(
      (info) => Date.now() - info.timestamp < 10000
    ); // Last 10 seconds
  ...
  async generateLeakReport() {
    const undisposed = this.getUndisposedMaterials();
    const recentLeaks = undisposed.filter(
      (info) => Date.now() - info.timestamp < 30000
    ); // Last 30 seconds
    ...
    return {
      timestamp: new Date().toISOString(),
      severity:
        undisposed.length > 500
          ? "CRITICAL"
          : undisposed.length > 200
          ? "HIGH"
          : "MEDIUM",
      totalUndisposed: undisposed.length,
      recentLeaks: recentLeaks.length,
