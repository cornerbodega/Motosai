...
export class SimpleBikePhysics {
  ...
  setBikeStats(stats) {
    ...
    this.acceleration =
      this.baseAcceleration * this.bikeStats.accelerationMultiplier;
  }
  ...
  updateSpeed(deltaTime) {
    ...
    const effectiveBrake =
      this.controls.brake > threshold ? this.controls.brake : 0;
    const effectiveThrottle =
      this.controls.throttle > threshold ? this.controls.throttle : 0;
    ...
    // Apply throttle (only if not braking)
      ...
      if (displaySpeedMPH < 60) {
        ...
        dragCoefficient = Math.max(
          0.3,
          0.5 * Math.exp(-displaySpeedMPH / 1000)
        );
      }
    ...
    const calculatedRPM =
      (this.speed * gearRatios[this.gear] * finalDrive * primaryRatio * 60) /
      wheelCircumference;
  ...
  updateTurning(deltaTime) {
    ...
    if (Math.abs(steerInput) < deadzone) {
      ...
      steerInput =
        (Math.sign(steerInput) * (Math.abs(steerInput) - deadzone)) /
        (1 - deadzone);
    }

    // Trail braking effect - off throttle progressively tightens turn radius
    if (Math.abs(steerInput) > 0.1 && this.speed > 5) {
      // Only when turning and moving
      if (this.controls.throttle < 0.3) {
        ...
        this.trailBrakeMultiplier +=
          (targetMultiplier - this.trailBrakeMultiplier) * deltaTime * 8;
      } else {
        // On throttle - gradually release trail brake effect
        this.offThrottleTime = Math.max(
          0,
          this.offThrottleTime - deltaTime * 3
        ); // Faster release
        this.trailBrakeMultiplier +=
          (1.0 - this.trailBrakeMultiplier) * deltaTime * 4;
      }
    } else {
      // Not turning - reset
    ...
    const targetTurnSpeed =
      -steerInput *
      this.maxTurnSpeed *
      speedFactor *
      this.trailBrakeMultiplier *
      this.bikeStats.handlingMultiplier;
    ...
    if (Math.abs(steerInput) < 0.01) {
      ...
      const turnChange =
        Math.sign(turnDiff) *
        Math.min(Math.abs(turnDiff), this.turnAcceleration * deltaTime);
    ...
    const trailBrakeLean =
      this.trailBrakeMultiplier > 1.0
        ? (this.trailBrakeMultiplier - 1.0) * 0.3
        : 0;
    ...
    const leanChange =
      Math.sign(leanDiff) * Math.min(Math.abs(leanDiff), 3.0 * deltaTime);
    this.leanAngle += leanChange;

    // Clamp lean angle
    this.leanAngle = Math.max(
      -this.maxLeanAngle,
      Math.min(this.maxLeanAngle, this.leanAngle)
    );
  ...
  updateVisuals(deltaTime) {
    ...
    if (this.controls.throttle > 0) {
      ...
      this.rotation.pitch +=
        (targetPitch - this.rotation.pitch) * 5 * deltaTime;
    ...
    if (this.collision.isWobbling && this.collision.wobbleAmplitude > 0.01) {
      ...
      const wobble =
        Math.sin(Date.now() * 0.001 * wobbleFreq) *
        this.collision.wobbleAmplitude *
        0.3; // Also reduced amplitude
  ...
  checkBarrierCollisions() {
    ...
    if (Math.abs(this.position.x) >= railingPosition - 0.2) {
      // Small buffer for instant death
      // Calculate impact speed for collision effects
      const impactSpeed =
        Math.abs(this.velocity.x) + Math.abs(this.velocity.z * 0.1); // Total impact velocity
  ...
  applyCollisionResponse(angle, speed, vehicle) {
    ...
    if (response.crash && displaySpeedMPH > 20) {
      // Crash at 20mph+ for better gameplay
      this.collision.isCrashed = true;
      this.collision.recoveryTime = 2.0;
