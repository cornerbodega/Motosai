...
export class MotorcyclePhysicsV2 {
  ...
  calculateTireForcesV2(speed) {
    ...
    const longTransfer =
      (longAccel * this.mass * this.cogHeight) / this.wheelbase;

    // Lateral weight transfer (affects grip during lean)
    const latTransfer =
      ((latAccel * this.mass * this.cogHeight) / this.wheelbase) * 0.3;

    // Calculate wheel loads
    this.frontWheel.load =
      this.mass * this.gravity * 0.45 - longTransfer - latTransfer;
    this.rearWheel.load =
      this.mass * this.gravity * 0.55 + longTransfer + latTransfer;
    ...
    if (isBurnoutCondition) {
      ...
      this.engine.burnoutSmoke = Math.min(
        1,
        this.engine.burnoutSmoke + this.dt * 2
      ); // Ramp up smoke
      ...
      this.engine.rpm = Math.min(
        this.engine.maxRPM - 500,
        this.engine.rpm + this.dt * 8000
      );
    } else {
      // Normal driving
      this.engine.isBurnouting = false;
      this.engine.burnoutSmoke = Math.max(
        0,
        this.engine.burnoutSmoke - this.dt * 4
      ); // Fade smoke quickly
      ...
      if (this.rearWheel.load > 0) {
        ...
        const actualForce =
          Math.min(Math.abs(driveForce), maxForce) * Math.sign(driveForce);
        ...
        if (Math.abs(this.rotation.roll) > 0.05 && this.controls.throttle > 0) {
          ...
          const straighteningMoment =
            actualForce * Math.abs(this.rotation.roll) * 0.15;
    ...
    if (
      (this.controls.frontBrake > 0 || this.controls.rearBrake > 0) &&
      Math.random() < 0.05
    ) {
    }
    ...
    if (speed > 0.1) {
      const brakingX =
        -(this.velocity.x / speed) * (frontBrakeForce + rearBrakeForce);
      const brakingZ =
        -(this.velocity.z / speed) * (frontBrakeForce + rearBrakeForce);
      forces.x += brakingX;
      forces.z += brakingZ;
    }
  ...
  calculateCorneringForces(speed) {
    ...
    if (Math.abs(leanAngle) > 0.01) {
      ...
      const progressiveTurnFactor =
        1 + this.turning.progressiveLeanFactor * 1.0; // Double the effect
      const effectiveLeanAngle =
        leanAngle * progressiveTurnFactor * throttleRadiusMultiplier;
      // Reduce turn radius calculation for tighter turns
      const turnRadius =
        (speed * speed * 0.6) /
        (this.gravity * Math.tan(Math.abs(effectiveLeanAngle)));
      ...
      const slipAngle =
        Math.atan2(this.velocity.x, this.velocity.z) - this.rotation.yaw;
      const slipForce =
        Math.sin(slipAngle) * speed * this.mass * 0.5 * slipReduction;
  ...
  calculateCamberThrust() {
    ...
    forces.x =
      camberForce * Math.sin(perpAngle) * Math.sign(this.rotation.roll);
    forces.z =
      camberForce * Math.cos(perpAngle) * Math.sign(this.rotation.roll);
  ...
  calculateGyroscopicEffects(speed) {
    ...
    const frontWheelGyro =
      this.frontWheel.momentOfInertia * this.frontWheel.angularVelocity;
    const rearWheelGyro =
      this.rearWheel.momentOfInertia * this.rearWheel.angularVelocity;

    // When leaning, gyroscopic effect wants to turn the bike
    torques.yaw +=
      (frontWheelGyro + rearWheelGyro) *
    ...
    if (this.controls.throttle > 0) {
      const accelerationStability =
        this.controls.throttle * (frontWheelGyro + rearWheelGyro) * 0.2;
  ...
  calculateSteeringTorques(speed) {
    ...
    const isAccelerating = this.controls.throttle > 0;
    const throttleModifier = isAccelerating
      ? 1.0 - this.controls.throttle * 0.2
      : 1.5; // Better turning off throttle

    // Apply sharp turn boost from tap-then-hold OR brake-turning
    // Brake turn boost: 1.0 + (boost * multiplier)
    const brakeTurnBoost =
      1.0 + this.turning.brakeTurnBoost * this.turning.brakeTurnMultiplier;
    const tapTurnBoost = this.turning.sharpTurnActive
      ? this.turning.sharpTurnBoost
      : 1.0;
    ...
    if (isAccelerating && Math.abs(this.rotation.roll) > 0.05) {
      ...
      const straighteningTorque =
        -this.rotation.roll * this.controls.throttle * 8000 * (speed / 30);
    ...
    if (speed > this.turning.counterSteerThreshold) {
      ...
      const progressiveMultiplier =
        (0.85 + 0.15 * this.turning.progressiveLeanFactor) *
        throttleModifier *
        sharpTurnMultiplier;
      const steerTorque =
        this.controls.steerInput * 25000 * (speed / 25) * progressiveMultiplier; // MUCH higher torque
      ...
      const leanAngle = this.rotation.roll;
      const turnRate =
        Math.sin(leanAngle) * Math.sqrt(this.gravity / (speed + 0.5)); // Less speed denominator
      const turnEnhancement =
        (1.5 + this.turning.progressiveLeanFactor * 1.0) *
        throttleModifier *
        sharpTurnMultiplier; // Start at 1.5x
      ...
      const trailReduction =
        (1 - this.turning.progressiveLeanFactor * 0.4) *
        (isAccelerating ? 1.0 : 0.7);
      const trailTorque =
        -this.frontWheel.steerAngle *
        speed *
        this.turning.trailEffect *
        100 *
        trailReduction;
      ...
      const progressiveMultiplier =
        (0.7 + 0.3 * this.turning.progressiveLeanFactor) * throttleModifier;
    ...
    const throttleLeanReduction = isAccelerating
      ? 1.0 - this.controls.throttle * 0.2
      : 1.0; // Less reduction
    const progressiveLeanMultiplier =
      (1.5 + this.turning.progressiveLeanFactor * 1.2) *
      throttleModifier *
      throttleLeanReduction *
      sharpTurnMultiplier;
    const targetLean =
      this.controls.lean * this.maxLeanAngle * progressiveLeanMultiplier;
    ...
    const progressiveResponse =
      (2.0 + this.turning.progressiveLeanFactor * 1.5) *
      throttleModifier *
      sharpTurnMultiplier; // MUCH more aggressive
    const leanCorrection =
      leanError * 10000 * speedFactor * progressiveResponse; // MUCH higher response
    ...
    const dampingReduction =
      (1 - this.turning.progressiveLeanFactor * 0.5) *
      (isAccelerating ? 0.7 : 0.5) *
      sharpTurnDampingMultiplier;
  ...
  updateBrakeHoldTime() {
    ...
    const normalized =
      this.turning.brakeHoldTime / this.turning.maxBrakeHoldTime;
  ...
  updateTurnHoldTime() {
    ...
    if (Math.abs(this.controls.lean) > 0) {
      // Currently turning
      if (
        currentTurnDirection === this.turning.lastTurnDirection ||
    ...
    if (normalized < 0.1) {
      ...
      this.turning.progressiveLeanFactor =
        0.4 + 0.6 * Math.sin((adjustedNorm * Math.PI) / 2);
    }
  }
  ...
  updateSuspension() {
    ...
    this.suspension.front.compression = Math.max(
      0,
      Math.min(1, 0.5 + brakeG * 0.4 * (1 - antiDiveReduction))
    );
    ...
    this.suspension.rear.compression = Math.max(
      0,
      Math.min(1, 0.5 + accelG * 0.4 * (1 - antiSquatReduction))
    );

    // Pitch from suspension
    const pitchFromSuspension =
      (this.suspension.rear.compression - this.suspension.front.compression) *
      0.15;
  ...
  updateTirePhysics() {
    ...
    const leanFactor =
      1 - (Math.abs(this.rotation.roll) / this.maxLeanAngle) * 0.2;
  ...
  updateEngine() {
    ...
    if (throttle > 0.05 && this.controls.lastThrottle < 0.05) {
      ...
      this.controls.throttleBoostTimer = Math.max(
        0,
        this.controls.throttleBoostTimer - this.dt
      );
    } else {
    ...
    if (throttle > 0) {
      ...
      if (this.controls.throttleBoostTimer > 0) {
        const boostFactor =
          1.0 + (this.controls.throttleBoostTimer / 0.8) * 0.5; // Up to 50% boost
    ...
    const torque =
      this.calculateEngineTorque(this.engine.rpm) * throttleResponse;

    // No engine braking - coasts freely like automatic or with clutch in
    this.rearWheel.torque =
      torque * gearRatio * this.engine.finalDrive * this.engine.clutch;
  }
  ...
  setControls(controls) {
    ...
    if (speed > this.turning.counterSteerThreshold) {
      ...
      this.controls.steerInput =
        -controls.lean * 0.5 || this.controls.steerInput;
    } else {
